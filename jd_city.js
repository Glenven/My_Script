...
let inviteCodes = ['-ryUXq8FZWBLYBPFG4nH_tmiGkSznnKG']
let inviteCode='';
!(async () => {
  if (!cookiesArr[0]) {
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `https://cdn.jsdelivr.net/gh/Glenven/MyUpdateTeam@main/shareCodes/jd_city.json`, 'timeout': 15000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
