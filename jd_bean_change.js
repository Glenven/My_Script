      ...
      try {
        if (err) {
          ...
          if (data) {
            ...
            $.message += `\n京喜红包：${$.jxRed}(即将过期${$.jxRedExpire.toFixed(2)})元\n极速红包：${$.jsRed}(即将过期${$.jsRedExpire.toFixed(2)})元\n京东红包：${$.jdRed}(即将过期${$.jdRedExpire.toFixed(2)})元\n当前总红包：${$.balance}(今日总过期${$.expiredBalance})元\n`;
          } else {
            console.log(`京东服务器返回空数据`)
          }
        }
