...
*/
const $ = new Env('东东农场');
let cookiesArr = [], cookie = '', cookienameArr = [], cookiename = '', jdFruitShareArr = [], isBox = false, notify, newShareCodes, allMessage = '';
  ...
  try {
    ...
    if ($.farmInfo.farmUserPro) {
      ...
      if ($.farmInfo.treeState === 2 || $.farmInfo.treeState === 3) {
        ...
        message += `【提醒⏰】${$.farmInfo.farmUserPro.name}已可领取\n`;
        // if ($.isNode()) {
...
async function doTenWater() {
  ...
  if ($.farmTask.totalWaterTaskInit.totalWaterTaskTimes < $.farmTask.totalWaterTaskInit.totalWaterTaskLimit) {
    ...
    if (isFruitFinished) {
      option['open-url'] = urlSchema;
      $.msg($.name, ``, `【提醒⏰】${$.farmInfo.farmUserPro.name}已可领取\n请去京东APP或微信小程序查看\n点击弹窗即达`, option);
      $.done();
      if ($.isNode()) {
