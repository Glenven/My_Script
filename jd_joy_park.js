...
async function showMsg() {
    if ($.isNode() && allMessage) {
      await notify.sendNotify(`${allMessage}`,`请前往京东极速版APP查看使用优惠券\n活动入口：京东极速版APP->我的->汪汪乐园`);
    }
}
        ...
        if (err) {
          ...
          if (printLog) {
            ...
            if (data.data.level >= 30 && $.isNode()) {
              allMessage +=`${$.name} - 账号${$.index} - ${$.nickName}`, `【京东账号${$.index}】${$.nickName || $.UserName}\n当前等级: ${data.data.level}\n已达到单次最高等级奖励\n`
...
async function doJoyMergeAll(activityJoyList) {
  ...
  if (joyMinLevelArr.length >= 3) {
    $.log(`开始合成 ${minLevel} ${joyMinLevelArr[0].id} <=> ${joyMinLevelArr[1].id} 【限流严重，8秒后合成！如失败会重试】`);
    await $.wait(8000)
