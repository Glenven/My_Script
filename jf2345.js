...
function PostUrl(api,params,body){
    return {
        ...
        headers:{
            ...
            'User-Agent': 'Mozilla/5.0 (Linux; Android 10; COL-AL10 Build/HUAWEICOL-AL10; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/74.0.3729.186 Mobile Safari/537.36 package/com.shouji2345 versionName/4.6.4 versionCode/464 WangPai,WangPaiBrowser,Android',
      ...
      try {
        if (err) {
            ...
            if(data.status == "200"){
                ...
                console.log(`[利润报告] 请求信息：${data.message} `)
    ...
    $.get(GetUrl(`jifen/entry${params(obj)}`),(err, resp, data) => {
      try {
        if (err) {
            ...
            console.log(`[积分统计] 请求信息：${data.message} `)
