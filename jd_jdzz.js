...
!(async () => {
  ...
  $.authorTuanList = [];
  await requireConfig();
  if (helpAuthor) await getAuthorShareCode('https://cdn.jsdelivr.net/gh/h455257166/MyUpdateTeam@main/shareCodes/jd_zz.json');
  if (!cookiesArr[0]) {
  ...
  console.log(`\n\n开始账号内部互助 【赚京豆(微信小程序)-瓜分京豆】活动(优先内部账号互助(需内部cookie数量大于${$.assistNum || 4}个)，如有剩余助力次数则给作者助力)\n`)
  for (let i = 0; i < cookiesArr.length; i++) {
    $.canHelp = true
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
      ...
      if ($.canHelp) {
        console.log(`开始账号内部互助 赚京豆-瓜分京豆 活动，如有剩余则给作者助力`)
